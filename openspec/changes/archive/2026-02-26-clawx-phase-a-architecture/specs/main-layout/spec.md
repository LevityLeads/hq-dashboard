...
- **WHEN** 用户访问任何管控页面路由
- **THEN** `ConsoleLayout` SHALL 渲染以下结构：TopBar（复用现有组件）在顶部，主内容区域填充剩余空间并支持滚动
...
- **WHEN** 管控页面内容渲染
- **THEN** 内容区域 SHALL 具有适当的内边距（`p-6`）、最大宽度约束（防止超宽屏内容过于拉伸）和垂直滚动支持
...
- **THEN** TopBar SHALL 显示：「返回 Office」按钮（左侧）、当前页面标题、「控制台」菜单（用于页面间切换）、连接状态。SHALL NOT 显示 2D/3D 切换和 Agent 指标
